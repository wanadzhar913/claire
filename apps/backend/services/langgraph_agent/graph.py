...
from backend.services.langgraph_agent.tools import tools
# from backend.core.logging_config import logger
from backend.services.langgraph_agent.prompts import load_system_prompt
...
class LangGraphAgent:
    ...
    async def _get_connection_pool(self) -> AsyncConnectionPool:
        ...
        if self._connection_pool is None:
            try:
                ...
                # logger.info("connection_pool_created", max_size=max_size, environment=settings.BACKEND_API_ENVIRONMENT)
    ...
    async def _update_long_term_memory(self, user_id: str, messages: list[dict], metadata: dict = None) -> None:
        ...
        try:
            ...
            #     error=str(e),
            # )
            # raise Exception(f"failed to update long term memory: {str(e)}")
            pass
        ...
        if self._graph is None:
            try:
                ...
                self._graph = graph_builder.compile(
                    checkpointer=checkpointer, name=f"{settings.BACKEND_PROJECT_NAME} Agent ({settings.BACKEND_API_ENVIRONMENT})"
                )

                # logger.info(
                #     "graph_created",
                #     graph_name=f"{settings.BACKEND_PROJECT_NAME} Agent",
                #     environment=settings.BACKEND_API_ENVIRONMENT,
                #     has_checkpointer=checkpointer is not None,
                # )
            except Exception as e:
                # logger.error("graph_creation_failed", error=str(e), environment=settings.BACKEND_API_ENVIRONMENT)
    ...
    async def get_stream_response(
        ...
        config = {
            ...
            # "callbacks": [
            #     CallbackHandler(
            #         environment=settings.BACKEND_API_ENVIRONMENT, debug=False, user_id=user_id, session_id=session_id
            #     )
            # ],
            "metadata": {
                "user_id": user_id,
