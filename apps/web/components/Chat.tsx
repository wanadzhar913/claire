    ...
    <Card className="min-w-0">
      ...
      <CardContent className="min-w-0">
        {error ? (
          <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm">
            ...
            <div className="text-muted-foreground mt-1 wrap-break-word">{error.message}</div>
      ...
      <CardFooter className="flex-col items-stretch gap-2">
        ...
        <form onSubmit={onSubmit} className="flex w-full items-end gap-2">
          ...
          {isBusy ? (
            <Button type="button" variant="outline" onClick={() => stop()} className="h-[44px] shrink-0">
              <Square className="h-4 w-4" />
              Stop
            </Button>
          ) : (
            <Button type="submit" className="h-[44px] shrink-0" disabled={!input.trim()}>
