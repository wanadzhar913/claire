...
function toBackendMessages(messages: UIMessage[]): Array<{ role: "user" | "assistant" | "system"; content: string }> {
  return (messages || [])
    .map((m) => ({ role: m.role, content: extractText(m).trim() }))
    .filter((m) => m.content.length > 0);
}

async function* parseBackendSse(stream: ReadableStream<Uint8Array>): AsyncGenerator<BackendStreamEvent> {
  ...
  while (true) {
    ...
    while (true) {
      ...
      try {
        ...
        if (typeof evt?.content === "string" && typeof evt?.done === "boolean") {
          yield evt;
        }
      } catch {
...
export async function POST(req: Request): Promise<Response> {
  const payload = (await req.json().catch(() => null)) as { messages?: UIMessage[] } | null;
  ...
  if (!backendRes.ok) {
    ...
    return new Response(text || `Backend error (${backendRes.status})`, { status: 502 });
  }
